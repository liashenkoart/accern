...
const RequestTrial = ({  }) => {
  ...
  return (
    <Modal className="modal-custom" show={modals.isRequestTrial} onHide={()=>dispatchModals({type:"SET_MODALS", data:{isRequestTrial:false}})} size="xl" aria-labelledby="contained-modal-title-vcenter" centered>
      <Modal.Body>
      <div className="modal-close" onClick={()=>dispatchModals({type:"SET_MODALS", data:{isRequestTrial:false}})}><Icon variant="icon-close" /></div>
        <Container>
          <Row>
            <Col lg={7} className="modal-padding bg-white">
              <FormComp from="Home page" data={settings.requestTrial.form} />
            </Col>
